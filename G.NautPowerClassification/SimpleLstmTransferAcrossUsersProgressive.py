...
if __name__ == '__main__':
    ...
    for testUser in ['ryan','jhony','jackie','juan']:
        ...
        try:
                ...
                # axes[0].set_title('{:}_test_before'.format(testUser))
                ...
                # axes[1].legend()
                # # plt.show()
                # plt.savefig(plotImageRootName + '{:}_test_before'.format(testUser))
                # plt.close(fig)
                ...
                for i in range(3):
                    ...
                    # # Save training plot
                    # fig, axes = plt.subplots(2, 1, sharex=True)
                    # axes[0].set_title('{:}_reTrain_{:d}'.format(testUser,i))
                    # axes[0].plot(transfer_history.history['acc'], label='train acc')
                    # axes[0].plot(transfer_history.history['val_acc'], label='val acc')
                    # axes[0].plot(transferEarlyStopCallback.validationAcc2, label='test acc')
                    # axes[0].set_ylim([0.5, 1])
                    # axes[0].axvline(x=transferEarlyStopCallback.epochOfMaxValidation)
                    # axes[1].plot(transfer_history.history['loss'], label='train loss')
                    # axes[1].plot(transfer_history.history['val_loss'], label='val loss')
                    # axes[1].plot(transferEarlyStopCallback.validationAcc2, label='test loss')
                    # axes[0].legend()
                    # axes[1].legend()
                    # # plt.show()
                    # plt.savefig(plotImageRootName + '{:}_reTrain_{:d}.png'.format(testUser,i))
                    # plt.close(fig)
                    #
                    # #Set best Weights
