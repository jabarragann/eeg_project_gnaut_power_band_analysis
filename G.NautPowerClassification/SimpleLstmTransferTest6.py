...
if __name__ == '__main__':
    ...
    resultsContainer2 = []
    resultsPath = './results/results_transfer6/'
    dataPath = './data/DifferentWindowSizeData/'
    windowSize = [10,15,20,5]
    lstmTimesteps = [6,4,3,12]
    # windowSize = [10,15]
    # lstmTimesteps = [6,4]

    for w1,t1 in zip(windowSize, lstmTimesteps):
        makeDir( resultsPath + '{:02d}s/'.format(w1))
        resultsPath2 = resultsPath + '{:02d}s/'.format(w1)
        dataPath2 = dataPath + '{:02d}s/'.format(w1)

        for user in [ 'jhony','jackie','ryan','juan']:

            resultsFileName = resultsPath2+'{:}_test.csv'.format(user)
            resultsContainer = []
            plotImageRootName = resultsPath2 + '{:}_test_'.format(user)

            #Get Data
            dataContainer = lstmClf.getDataSplitBySessionByTrial(dataPath2+'/{:}/'.format(user),
                                                                 timesteps=t1,
            ...
            try:
                ...
                for testingKey in dataContainer.keys():
                    ...
                    for lt,ht in arr[idx]:
                        ...
                        data = {testingKey: [w1,user,testingKey,validationSession,
                                             lt,ht, max(history.history['val_acc']), evalTest0[1]]}
                        df1 = pd.DataFrame.from_dict(data, orient='index',
                                                     columns=['Window_size','user','test_session','validation_session',
                                                              'low_trial','high_trial','validationAcc','testAcc'])
                ...
                try:
                    ...
                    finalDf.to_csv(resultsPath2 + 'temp_{:}.csv'.format(user), index=False)
