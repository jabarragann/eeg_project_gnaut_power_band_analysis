...
df = pd.read_csv('barchart-two-conditions2.csv', sep=',')
df = df.loc[:, ~df.columns.str.contains('^Unnamed')]
#df  = df1.sort_values('Users',ascending=True)

## Create lists for the plot
conditions = df['Row Labels']
x_pos = np.arange(len(conditions))
beforeAcc = df['SimpleMean'].values
stdBefore = df['SimpleStd'].values
afterAcc = df['OptimizedMean'].values
stdAfter = df['OptimizedStd'].values
...
# Make the plot
fig, ax = plt.subplots()
ax.set_title("Training with the first sessions only")
