...
if __name__ == '__main__':
    ...
    LSTM_SAMPLE_SIZE = [110]
    WINDOW_SIZE = [10]
    RESULTS_ROOT = Path('./results/EegResults/results_transfer10/') / 'aa13a_pyprep_complete'
    SOURCE_PATH = Path('./data/de-identified-pyprep-dataset-reduced-critically')
    ...
    for lstmSampleSize,windowSize in itertools.product(LSTM_SAMPLE_SIZE,WINDOW_SIZE):
        ...
        for user in USERS:
            dataPath = SOURCE_PATH /'{:02d}s/'.format(windowSize)
            ...
            # results = transferModule.experimentalTransferCrossValidation(lstmSteps, dataPath,resultsPath,
            #                                                  user,USERS,eegChannels=EEG_CHANNELS,
            #                                                  powerCoefficients=POWER_COEFFICIENTS)
