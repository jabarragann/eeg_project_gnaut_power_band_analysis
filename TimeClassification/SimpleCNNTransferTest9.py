'''
Grid search for the best CNN models sample size and windows size.
...
if __name__ == '__main__':
    ...
    paramsDict = {'TESTED_USERS': ['karuna','santy','juan', 'jackie', 'ryan','jhony'], #
              'ALL_USERS': ['juan', 'jackie', 'ryan','karuna','santy','jhony'], #,
              ...
              'WINDOW_SIZE': [10,20],
              'RESULTS_ROOT': Path('.').resolve() / 'results/results_transfer9/aa1_pyprep/',
              'AIM':"The aim of the experiment is to evaluate the performance of the time domain classifiers."
              }
    ...
    for lstmSampleSize,windowSize in itertools.product(params.LSTM_SAMPLE_SIZE, params.WINDOW_SIZE):
        ...
        for user in params.TESTED_USERS:
            ...
            results = crossValidationModule.userCrossValidationMultiUser(lstmSteps, dataPath,resultsPath,
                                                                         ...
                                                                         powerCoefficients=params.POWER_COEFFICIENTS,dataFormat='time')
